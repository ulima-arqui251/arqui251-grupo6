...
**Contexto:**  
El módulo de exploración musical debe permitir búsquedas eficientes sobre entidades musicales como artistas, álbumes y géneros. Estas búsquedas incluyen filtros por nombre, tipo, etiquetas y combinaciones múltiples, además de la visualización ágil de resultados. Al trabajar con MongoDB como base de datos documental, es necesario definir cómo se modelarán y organizarán los documentos para soportar estas consultas sin afectar el rendimiento.
...
2. **Documentos embebidos por jerarquía de uso:**
   - Incluir información de álbumes dentro del documento del artista, o canciones dentro del álbum.
...
**Decisión:**  
Adoptar un modelo mixto con desnormalización parcial.

**Sustento:**  
La exploración musical requiere alta velocidad de lectura y respuesta inmediata en filtros múltiples. Embebiendo datos como nombre de artista o género dentro de documentos de álbumes y canciones, se optimiza el rendimiento para estos casos comunes sin depender de *joins*. Al mismo tiempo, se mantiene la capacidad de actualizar información centralizada mediante referencias. Esta estrategia favorece la experiencia del usuario final sin comprometer la mantenibilidad ni la escalabilidad del sistema.
