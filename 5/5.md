...
Sin embargo, de manera adicional se realizó las presentes tablas para resumir nuestras tácticas de arquitectura, las cuales son decisiones adicionales de arquitectura argumentadas con ADR.
...
| ESC-GU-02 | Disponibilidad | Auto Scaling con Kubernetes | 1. Escalado Vertical, 2. Auto Scaling Horizontal | 2. Auto Scaling Horizontal |
...
| ESC-GU-08 | Seguridad | CAPTCHA progresivo + Redis Rate Limiting | 1. Rate Limiting Simple, 2. Sistema Adaptativo | 2. Sistema Adaptativo |
| ESC-GU-09 | Seguridad | Auth0 Passwordless / Reset integrado | 1. Tokens UUID, 2. JWT + OTP de un solo uso, 3. Auth0 Passwordless / Reset integrado | 3. Auth0 Passwordless / Reset integrado |
| ESC-GU-10 | Usabilidad | Formulario por pasos con validación en tiempo real | 1. Formulario monolítico, 2. Formulario por pasos | 2. Formulario por pasos |
| ESC-GU-11 | Usabilidad | Asistente paso a paso con validación | 1. Email tradicional, 2. Asistente interactivo | 2. Asistente interactivo |
| ESC-GU-12 | Capacidad de ser probado | Pruebas unitarias automatizadas | 1. Pruebas Manuales, 2. Pruebas Automatizadas | 2. Pruebas Automatizadas |
| ESC-GU-13 | Capacidad de ser probado | Pruebas con matriz de casos | 1. Pruebas Unitarias Individuales, 2. Pruebas con Matriz | 2. Pruebas con Matriz |
...
| ESC-VP-05 | Interoperabilidad | API Gateway con GraphQL | 1. Llamadas directas al microservicio de recomendaciones, 2. Capa intermedia de API Gateway | 2. API Gateway |
...
| ESC-VP-07 | Rendimiento | GraphQL + DataLoader | 1. REST API tradicional con joins en backend, 2. GraphQL con batching y caching | 2. GraphQL Optimizado |
...
| ESC-GB-09 | Seguridad | Backend personalizado con JWT para autenticación | 1. Firebase Authentication + Firestore, 2. Backend personalizado (Node.js + Express + JWT + PostgreSQL) | 2. Backend personalizado (Node.js + Express + JWT + PostgreSQL) |
| ESC-GB-10 | Seguridad | Encriptación nativa de MongoDB | 1. Encriptación a nivel aplicación, 2. Encriptación nativa de MongoDB | 2. Encriptación nativa de MongoDB |
| ESC-GB-11 | Usabilidad (Iniciativa del usuario) | Cálculo incremental con operadores atómicos de MongoDB | 1. Recálculo completo del promedio, 2. Triggers en PostgreSQL, 3. Operadores atómicos de MongoDB (inc,avg) | 3. Operadores atómicos de MongoDB |
| ESC-GB-12 | Escalabilidad | Escalado horizontal automático | 1. Escalamiento vertical, 2. Escalamiento horizontal manual, 3. Escalamiento horizontal automático | 3. Escalamiento horizontal automático |
...
| ESC-GR-07 | Rendimiento | Optimización algorítmica y caché inteligente | 1. Optimización de algoritmos, 2. Implementación de caché inteligente | 1. Optimización de algoritmos |
